...
for positions = 1:4
    ...
    % Add sub-folders
    addpath(genpath(pwd));
    posePub = rospublisher("/robot/newPose", "std_msgs/Float64MultiArray");
    progressPub = rospublisher("/robot/progressNow", "std_msgs/Int64");
    robotLocationSub = rossubscriber("/robot/locationCurrent", "std_msgs/Float64");
    ...
    % Conditional Statements determine which positon the object is in
    % reference to the robot
    positionChanger = positions;
    if positionChanger == 1.0
        % a sphere
        s = collisionSphere(0.1)
        Ts = trvec2tform([0.8 0 0.5])
        s.Pose = Ts;
        env = [env; {s}];
        clear s
    elseif positionChanger == 2.0
        % a sphere
        s = collisionSphere(0.1)
        Ts = trvec2tform([0 -0.8 0.5])
        s.Pose = Ts;
        env = [env; {s}];
        clear s
    elseif positionChanger == 3.0
        % a sphere
        s = collisionSphere(0.1)
        Ts = trvec2tform([-0.8 0 0.5])
        s.Pose = Ts;
    ...
    axis([-1 1 -1 1 -0.5 1.0]);
    view(45, 20);
    grid off
    
    
    %% Set up task-space path
    % Conditional Statements determine which path to take depending on the
    % object position in reference to the robot
    if positionChanger == 1
        taskInit = trvec2tform([0.6,0.15,0.65])*axang2tform([0 1 0 0]);
        taskMed1 = trvec2tform([0.6,0.15,0.35])*axang2tform([0 1 0 0]);
        taskMed2 = trvec2tform([0.6,0-0.15,0.65])*axang2tform([0 1 0 0]);
        taskFinal = trvec2tform([0.6,-0.15,0.35])*axang2tform([0 1 0 0]);
    elseif positionChanger == 2
        taskInit = trvec2tform([0.15,-0.6,0.65])*axang2tform([0 0 1 -pi/2]);
        taskMed1 = trvec2tform([0.15,-0.6,0.35])*axang2tform([0 0 1 -pi/2]);
        taskMed2 = trvec2tform([-0.15,-0.6,0.65])*axang2tform([0 0 1 -pi/2]);
        taskFinal = trvec2tform([-0.15,-0.6,0.35])*axang2tform([0 0 1 -pi/2]);
    elseif positionChanger == 3
        taskInit = trvec2tform([-0.6,0.15,0.65])*axang2tform([0 1 0 pi]);
        taskMed1 = trvec2tform([-0.6,0.15,0.35])*axang2tform([0 1 0 pi]);
        taskMed2 = trvec2tform([-0.6,0-0.15,0.65])*axang2tform([0 1 0 pi]);
        taskFinal = trvec2tform([-0.6,-0.15,0.35])*axang2tform([0 1 0 pi]);
    elseif positionChanger == 4
        taskInit = trvec2tform([0.15,0.6,0.65])*axang2tform([0 0 1 pi/2]);
        taskMed1 = trvec2tform([0.15,0.6,0.35])*axang2tform([0 0 1 pi/2]);
        taskMed2 = trvec2tform([-0.15,0.6,0.65])*axang2tform([0 0 1 pi/2]);
        taskFinal = trvec2tform([-0.15,0.6,0.35])*axang2tform([0 0 1 pi/2]);
    ...
    %Creates a matrix to house the joint states
    anglesToMoveTo = zeros(size(jointConfigArray(:,:)));
    for j=1:size(anglesToMoveTo,1)
        %Second For loop rounds angles between pi and -pi
        for i=1:6
            x = jointConfigArray(j,1:6);
            if x(1, i) > pi
                anglesToMoveTo(j,i) = -2*pi + x(1, i);
            elseif x(1, i) < -pi
                anglesToMoveTo(j,i) = 2*pi- x(1, i);
            else
                anglesToMoveTo(j,i) = x(1, i);
            end
        end
        %Publish joint state
        msg = rosmessage(posePub);
        msg.Data = anglesToMoveTo(j, 1:6);
        send(posePub, msg);
        %Wait for command to give next joint state
        robotLoc = receive(robotLocationSub);
        %Publish 0 so Python knows there are more joint states
        moveOn = 0;
        msg = rosmessage(progressPub);
        msg.Data = moveOn;
        send(progressPub, msg);
        
    end
    %Publish 1 so Python knows there are no more joint states
    moveOn = 1;
    msg = rosmessage(progressPub);
    msg.Data = moveOn;
    send(progressPub, msg);
    %Wait for robot to begin moving 
    locationOfRobot = receive(robotLocationSub);
    moveOn = 0;
    msg = rosmessage(progressPub);
    msg.Data = moveOn;
    send(progressPub, msg);
